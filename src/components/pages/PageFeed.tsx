...
export function PageFeed({
	...
	pageId: number;
	pageSlug: string;
	currentUser: { id: number; avatar: { photoSrc: string } | null; firstName: string | null; lastName: string | null; userName: string };
	isOwner: boolean;
}) {
		...
		<div className="space-y-4">
			...
			{localPosts.length === 0 ? (
				<div className="text-center py-16 text-muted-foreground border border-dashed rounded-xl">
					...
					{isOwner && (
						<p className="text-sm mt-1">Share something with your followers!</p>
					)}
				</div>
			) : (
				localPosts.map((post) => (
