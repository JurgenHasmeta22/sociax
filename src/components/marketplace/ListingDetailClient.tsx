              ...
              {images.map((_, i) => (
                <button
                  ...
                  className={`w-1.5 h-1.5 rounded-full transition-colors ${
                    i === active ? "bg-primary" : "bg-white/60"
                  }`}
                />
              ))}
            </div>
...
export function ListingDetailClient({
  ...
  const [messages, setMessages] = useState<Message[]>(initialListing.messages);
  ...
  const handleOffer = () => {
    ...
    startTransition(async () => {
      try {
        const offer = await makeOffer(listing.id, amount, offerMsg || undefined);
        ...
        toast.error(e instanceof Error ? e.message : "Failed to make offer");
      }
    });
  };

  const handleRespondOffer = (offerId: number, action: "Accepted" | "Declined") => {
    startTransition(async () => {
      try {
  ...
  const statusBadge =
    listing.status === "Active"
      ? { label: "Active", classes: "bg-green-500/10 text-green-600 border-green-200" }
      : listing.status === "Sold"
      ? { label: "Sold", classes: "bg-blue-500/10 text-blue-600 border-blue-200" }
      : { label: listing.status, classes: "bg-amber-500/10 text-amber-600 border-amber-200" };
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-6">
        ...
        <div className="space-y-5">
          ...
          <div>
            <div className="flex items-start gap-2 mb-1">
              <h1 className="text-xl font-bold flex-1 leading-tight">{listing.title}</h1>
              <Badge
                variant="outline"
            ...
            <div className="flex items-center gap-3 text-xs text-muted-foreground">
              ...
              <span className="flex items-center gap-1"><Heart className="h-3 w-3" />{listing._count.saves} saves</span>
              <span className="flex items-center gap-1"><Clock className="h-3 w-3" />{format(new Date(listing.createdAt), "MMM d, yyyy")}</span>
            </div>
          </div>
          ...
          {/* Description */}
          <div>
            <h3 className="font-semibold mb-1.5 text-sm">Description</h3>
          ...
          <div className="flex items-center gap-3">
            <Link href={`/profile/${listing.seller.userName}`}>
              <Avatar className="h-11 w-11 ring-2 ring-primary/20">
                <AvatarImage src={listing.seller.avatar?.photoSrc ?? undefined} />
            ...
            <div className="flex-1 min-w-0">
              <Link href={`/profile/${listing.seller.userName}`} className="font-semibold text-sm hover:underline">
                {displayName(listing.seller)}
              </Link>
              <p className="text-xs text-muted-foreground">
                {listing.seller._count.listings} active listing{listing.seller._count.listings !== 1 ? "s" : ""}
              </p>
            </div>
            <Link href={`/profile/${listing.seller.userName}`}>
          ...
          {!isMine && listing.status === "Active" && (
            <div className="flex gap-2">
              <Button
                ...
                size="icon"
                onClick={handleSave}
                className={saved ? "bg-rose-500 hover:bg-rose-600 border-rose-500" : ""}
              ...
              <Button
                ...
                disabled={!!myOffer && myOffer.status === "Pending"}
              >
          ...
          {isMine && (
            <Button
              ...
              onClick={() => {
                startTransition(async () => {
                  const { markAsSold } = await import("@/actions/marketplace.actions");
      ...
      <div className="mt-8 grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        {(isMine || offers.some((o) => o.buyer.id === currentUserId)) && (
          <div className="space-y-3">
            <h2 className="font-semibold flex items-center gap-2">
              <Tag className="h-4 w-4" />
              Offers
              {offers.length > 0 && (
                <span className="text-xs text-muted-foreground font-normal">({offers.length})</span>
              )}
            </h2>
            ...
            {offers.length === 0 ? (
              ...
              <div className="space-y-2">
                {offers
                  .filter((o) => isMine || o.buyer.id === currentUserId)
                  .map((offer) => {
                    const offerBadge =
                        ...
                        <CardContent className="pt-3 pb-3">
                          <div className="flex items-start gap-3">
                            <Avatar className="h-8 w-8 shrink-0">
                              <AvatarImage src={offer.buyer.avatar?.photoSrc ?? undefined} />
                              <AvatarFallback className="text-[10px] bg-primary text-primary-foreground">
                                {displayName(offer.buyer)[0]?.toUpperCase()}
                              </AvatarFallback>
                            </Avatar>
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2">
                                <span className="font-semibold text-sm">
                                  {offer.buyer.id === currentUserId ? "Your offer" : displayName(offer.buyer)}
                                </span>
                                <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${offerBadge}`}>
                                  {offer.status}
                                </span>
                              </div>
                              <p className="text-base font-bold text-primary">${offer.amount.toFixed(2)}</p>
                              {offer.message && (
                                <p className="text-xs text-muted-foreground mt-0.5">{offer.message}</p>
                              )}
                            </div>
                          </div>
        ...
        <div className="mt-10">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
            {relatedListings.map((l) => (
              <Link key={l.id} href={`/marketplace/${l.slug}`} className="group block">
                <Card className="overflow-hidden hover:shadow-md transition-shadow h-full border-border/50">
                  ...
                  <CardContent className="p-2">
                    ...
                    <p className="text-sm font-bold text-primary mt-0.5">
                      {l.isFree ? <span className="text-green-500">Free</span> : `$${l.price.toFixed(2)}`}
                    </p>
                  </CardContent>
                </Card>
              </Link>
      ...
      <Dialog open={offerOpen} onOpenChange={setOfferOpen}>
        <DialogContent className="sm:max-w-sm">
          ...
          <DialogFooter>
            ...
            <Button onClick={handleOffer} disabled={!offerAmount || parseFloat(offerAmount) <= 0}>
              Send offer
            </Button>
          </DialogFooter>
        </DialogContent>
