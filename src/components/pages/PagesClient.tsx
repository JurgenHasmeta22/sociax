...
import { Flag, Plus, Loader2, ChevronRight, ThumbsUp, Edit, Users } from "lucide-react";
...
return (
<Link href={`/pages/${page.slug}`} className="relative shrink-0 w-40 h-48 rounded-xl overflow-hidden block hover:opacity-90 transition-opacity">
{page.avatarUrl ? (
<img
src={page.avatarUrl}
alt={page.name}
...
<div className={page.avatarUrl ? "pt-7 px-4 pb-4" : "pt-4 px-4 pb-4"}>
<Link href={`/pages/${page.slug}`}>
<h3 className="font-bold text-sm truncate hover:underline">
{page.name}
</h3>
...
{featured.map((page) => (
<FeaturedPageCard key={page.id} page={page} />
))}
</div>
...
<div className="text-center py-16 text-muted-foreground">
<Flag className="h-12 w-12 mx-auto mb-4 opacity-20" />
<p className="font-medium text-lg">No pages yet</p>
