...
export function SearchModal({
					...
					for (const k of ALL_TABS) {
						if (k === tab) {
							(merged as Record<string, unknown[]>)[k] = [
								...
								...(more as Record<string, unknown[]>)[k],
							];
						}
					}
					merged.hasMore = {
						...prev.hasMore,
						[tab]: more.hasMore[tab],
					};
	...
	const totalResults = ALL_TABS.reduce(
		(sum, k) =>
			sum +
			((results as Record<string, unknown[]> | null)?.[k]?.length ?? 0),
		0,
	);
	...
	const tabCount = (tab: TabKey) => {
		...
		return (
			(results as Record<string, unknown[]> | null)?.[tab]?.length ?? 0
		);
	};
